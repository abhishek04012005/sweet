...
$(document).ready(function() {
    ...
    var btnOpen = $("#open");
    var btnReset = $("#reset");
    var loveSound = document.getElementById("loveSound");

    // Open the envelope and display hearts + play sound
    function openEnvelope() {
        envelope.addClass("open").removeClass("close");
        createHearts();
        loveSound.currentTime = 0; // rewind to start
        loveSound.play();
    }

    // Close the envelope and remove hearts + stop sound
    function closeEnvelope() {
        envelope.addClass("close").removeClass("open");
        $(".hearts").empty();
        loveSound.pause();
